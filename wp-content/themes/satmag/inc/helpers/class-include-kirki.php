...
if ( ! class_exists( 'Kirki' ) ) {
		...
		class Kirki_Installer_Section extends WP_Customize_Section {
			...
			protected function render() {
				...
				<li id="accordion-section-<?php echo esc_attr( $this->id ); ?>" class="<?php echo esc_attr( $classes ); ?>" style="border-top:none;border-bottom:1px solid #ddd;padding:7px 14px 16px 14px;text-align:right;">
					<?php if ( ! $installed ) : ?>
						<p style="text-align:left;margin-top:0;"><?php _e( 'To take advantage of Satmag theme\'s features in the customizer. We recommends the <a href="https://wordpress.org/plugins/kirki/" target="_blank">Kirki Toolkit</a> plugin.', 'satmag' ); ?></p>
						<a class="install-now button-primary button" data-slug="kirki" href="<?php echo esc_url_raw( $plugin_install_url ); ?>" aria-label="<?php esc_attr_e( 'Install Kirki Toolkit now', 'satmag' ); ?>" data-name="Kirki Toolkit">
							<?php esc_html_e( 'Install Now', 'satmag' ); ?>
						</a>
					<?php else : ?>
						<p style="text-align:left;margin-top:0;"><?php esc_attr_e( 'You have installed Kirki. Activate it to take advantage of this theme\'s features in the customizer.', 'satmag' ); ?></p>
						<a class="install-now button-secondary button change-theme" data-slug="kirki" href="<?php echo esc_url_raw( self_admin_url( 'plugins.php' ) ); ?>" aria-label="<?php esc_attr_e( 'Activate Kirki Toolkit now', 'satmag' ); ?>" data-name="Kirki Toolkit">
							<?php esc_html_e( 'Activate Now', 'satmag' ); ?>
						</a>
					<?php endif; ?>
				</li>
				<?php
	...
	if ( ! function_exists( 'satmag_installer_register' ) ) {
		/**
		 * Registers the section, setting & control for the kirki installer.
		 *
		 * @param object $wp_customize The main customizer object.
		 */
		function satmag_installer_register( $wp_customize ) {
		...
		add_action( 'customize_register', 'satmag_installer_register' );
	}
}
